...
import 'package:dartz/dartz.dart' show some, none, Option, optionOf;
...
class EditorBloc extends Bloc<EditorEvent, EditorState> {
  ...
  EditorBloc(
    ...
    on<SetCanvasKey>(
      (event, emit) async => await _handleSetCanvasKey(event, emit),
      transformer: droppable(),
    );
          ...
          if (previousEditor != state.editor) {
            emit(
              EditorState(
                ...
                canvasBackgroundImageFile: state.canvasBackgroundImageFile,
              ),
            );
          }
        },
    ...
    return state.selectedElement.fold<Future<void>>(
      ...
      (el) async {
        Option<File> fileOption = (await _filePicker.pickSingleFile(
          allowedExtensions: allowedTextFilesExtensions.unlock,
          initialDirectory: '/',
        ))
            .fold(
          () {
          ...
          (file) {
            ...
            final Element updatedElement = el.copyWith(
              properties: (el.properties as VariableTextProperties).copyWith(
                ...
                placeHolderText: file.path.split(Platform.pathSeparator).last,
              ),
            );
            emit(
              state.copyWith(
    ...
    state.selectedElement.fold(
      ...
      (el) {
        ...
        if (event.updatedSize.width < minElementSideSize) {
          updatedSize = Size(el.rect.size.width, updatedSize.height);
        }
        if (event.updatedSize.height < minElementSideSize) {
          updatedSize = Size(updatedSize.width, el.rect.size.height);
        }
