...
void main() {
  ...
  EditorState createEditorState({
    ...
    Option<material.GlobalKey>? canvasKey,
    Option<material.Color>? canvasBackgroundColor,
    Option<File>? canvasBackgroundImageFile,
      ...
      EditorState(
        ...
        isEditingTextElement: isEditingTextElement ?? false,
        canvasKey: canvasKey ?? none(),
        canvasBackgroundColor: canvasBackgroundColor ?? none(),
        canvasBackgroundImageFile: canvasBackgroundImageFile ?? none(),
      );
  ...
  group('StaticTextStyleChanged', () {
    ...
    blocTest<EditorBloc, EditorState>(
      ...
      expect: () {
        ...
        final expectedState2 = createEditorState(
  ...
  group('TextEditingModeChanged', () {
    ...
    blocTest<EditorBloc, EditorState>(
      'Should emit the correct state when this event is fired',
      seed: () => createEditorState(
  ...
  group('SetCanvasKey', () {
    final material.GlobalKey canvasKey = material.GlobalKey();
    blocTest<EditorBloc, EditorState>(
      'Should set the canvas size correctly.',
      seed: () => createEditorState(
        editor: Editor.fromSet({}),
        canvasKey: some(material.GlobalKey()),
      ),
      build: () => createEditorBloc(),
      act: (bloc) => bloc.add(SetCanvasKey(some(canvasKey))),
      expect: () => [
        EditorState.initial().copyWith(canvasKey: some(canvasKey)),
      ],
    );
  });
  group('ChangeCanvasBackgroundColor', () {
    blocTest<EditorBloc, EditorState>(
      'Should set the canvas color correctly.',
      seed: () => createEditorState(
        editor: Editor.fromSet({}),
      ),
      build: () => createEditorBloc(),
  ...
  group('ChangeCanvasBackgroundImage', () {
    late MockFilePicker mockFilePicker;
    File backgroundImageFile = File('bg.png');

    blocTest<EditorBloc, EditorState>(
      'Should set the canvas background image correctly.',
      seed: () => createEditorState(
        editor: Editor.fromSet({}),
      ),
      setUp: () {
    ...
    blocTest<EditorBloc, EditorState>(
      'Should not change the canvas background image if no file is chosen.',
      seed: () => createEditorState(
        editor: Editor.fromSet({}),
        canvasBackgroundImageFile: some(backgroundImageFile),
      ),
      setUp: () {
        // define the mocks
        mockFilePicker = MockFilePicker();
  ...
  group('RemoveCanvasBackgroundImage', () {
    blocTest<EditorBloc, EditorState>(
      'Should set the canvas background image correctly.',
      seed: () => createEditorState(
        editor: Editor.fromSet({}),
        canvasBackgroundImageFile: some(File('bg.png')),
      ),
      build: () => createEditorBloc(),
      act: (bloc) => bloc.add(const RemoveCanvasBackgroundImage()),
      expect: () => [
        EditorState.initial(),
      ],
    );
